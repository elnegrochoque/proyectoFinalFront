...
const EditarEvaluacion = () => {
  ...
  const consultarEvaluacion = async () => {
    try {
      ...
      if (respuesta.status === 200) {
        ...
        setDuracionEvaluacionMilisegundos(
          resultado.duracionEvaluacionMlisegundos
        );
      }
    } catch (error) {
      console.log(error);
  ...
  const handleSubmit = async (e) => {
    ...
    const fechaYHoraInicioEvaluacion = new Date(
      fechaInicioEvaluacionRef.current.value +
        "T" +
        horaInicioEvaluacionRef.current.value
    );
    const fechaYHoraFinEvaluacion = new Date(
      fechaFinEvaluacionRef.current.value +
        "T" +
        horaFinEvaluacionRef.current.value
    );
                ...
                <Form onSubmit={handleSubmit}>
                  ...
                  <Row className="mx-1">
                    <FormGroup className="mr-4">
                      ...
                      <Form.Control
                        type="date"
                        defaultValue={evaluacion.fechaInicioEvaluacion}
                        ref={fechaInicioEvaluacionRef}
                      ></Form.Control>
                    </FormGroup>
                    <FormGroup>
